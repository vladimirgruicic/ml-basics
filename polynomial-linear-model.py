...
y_pred_test_linear = linear_model.predict(X_test_poly)
                                        
# Evaluate linear model performance on test data
...
print("\nLinear Regression:")
print("MEan Squared Error (MSE) on Test Data:", mse_test_linear)
print("R-squared on Test Data:", r_squared_test_linear)

# Interpret the results
if mse_test_linear < baseline_mse:
    print("Linear model outperforms baseline.")
else:
    print("Baseline outperforms linear model.")

# Visualize the results
plt.scatter(X_test, y_test, label='Test data')
plt.plot(X_test, y_pred_test_linear, color='g', linestyle='-', linewidth=1, label='Predictions Linear')
plt.xlabel('X')
plt.ylabel('y')
plt.legend()
